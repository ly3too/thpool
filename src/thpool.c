...
void* worker (void *arg) {
    DEBUG_FUNC(printf, ("enter worker: %I64u.\n", pthread_self()));
    while (keepalive) {
        sem_wait(&has_jobs);
        DEBUG_FUNC(printf, ("thread %I64u try to get a job .\n", pthread_self()));
        ...
        if (new_job != NULL) {
            ...
            DEBUG_FUNC(printf, ("thread %I64u did nothing.\n", pthread_self()));
        }
    }

    DEBUG_FUNC(printf, ("thread %I64u exit.\n", pthread_self()));
    return NULL;
}
